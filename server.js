...
const http = require('http');
    ...
    setInterval(() => {
    http.get(`http://savt-dd.glitch.me/`);
    }, 55000);
...
client.on("guildMemberRemove", async member => {
  ...
  if (!d) {
    client.channels.get(kanal).send(`<a:exit:727275701568274504> <@${member.user.id}> Sunucudan Ayrıldı.| Davet Eden: [ **BULUNAMADI**]`);
    return;
  } else {
    client.channels.get(kanal).send(`<a:exit:727275701568274504> <@${member.user.id}> Sunucudan Ayrıldı.| Davet Eden: [ <@${sa.id}> ]`);
...
client.on("guildMemberAdd", async member => {
  member.guild.fetchInvites().then(async guildInvites => {
    ...
    client.channels.get(kanal).send(`<a:login:727275773416702013> : <@${member.user.id}> Sunucuya Katıldı | Davet Eden: <@${davetçi.id}> [**${sayı2}**]`);
    if (!veri) return;
...
const exit = client.emojis.get('727275701568274504');
const login = client.emojis.get('727275773416702013');
